...
def main(args):
    ...
    if args.generate_meshes:
        ...
        len_args_gen = len(args_gen_queue)
        for i, args_gen in enumerate(args_gen_queue):
            cmd = f'{args.executable_gen} {args_gen}'
            print(colored(f'{i + 1}/{len_args_gen}: Running "{cmd}"', 'cyan'))
            os.system(cmd)
            print(colored(f'{i + 1}/{len_args_gen}: Finished "{cmd}"', 'magenta'))
    ...
    if args.evaluate_meshes:
        ...
        args_eval_common = f'{args_eval_common} --evaluate-tea-early-exit'
        args_eval_common = f'{args_eval_common} --match-radii'
        args_eval_common = f'{args_eval_common} --vis-radii-test {vis_radii}'
        ...
        len_args_eval = len(args_eval_queue)
        for i, args_eval in enumerate(args_eval_queue):
            cmd = f'{args.executable_eval} {args_eval}'
            print(colored(f'{i + 1}/{len_args_eval}: Running "{cmd}"', 'cyan'))
            os.system(cmd)
            print(colored(f'{i + 1}/{len_args_eval}: Finished "{cmd}"', 'magenta'))
