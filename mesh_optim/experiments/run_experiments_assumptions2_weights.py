...
args_parser.add_argument('-o', '--output_dir', type=str,
                         default='./outputs/assumptions',
                         help='Path to the output directory.')

maps = [
    ('scene_sp_pol_01', 1.0),
    ('scene_sp_pol_01_simple-1', 1.0),
]
...
def main(args):
    ...
    if args.generate_meshes:
        ...
        args_gen_common = f'{args_gen_common} --draw'
    ...
    if args.evaluate_meshes:
        ...
        sample_dists = [1e-3 * (2 ** a) for a in range(0, 16)]
        sample_dists.reverse()
        sample_dists.append(-1)
        for sd in sample_dists:
            ...
            if sd == -1:
                args_eval_common = f'{args_eval_common} --save-avg-expansions'
                ...
                args_eval_common = f'{args_eval_common} --compute-expansion-estimate {sd}'
