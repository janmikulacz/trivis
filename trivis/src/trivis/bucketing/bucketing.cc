...
std::optional<int> bucketing::FindTriangle(
    ...
    for (int x_neigh: {x, x - 1, x + 1}) {
        if (!(0 <= x_neigh && x_neigh < buckets.ncol())) continue;
        for (int y_neigh: {y, y - 1, y + 1}) {
            if (!(0 <= y_neigh && y_neigh < buckets.nrow())) continue;
...
int Bucketing::ToIdxX(double x) const {
    return ::ToIdxX(x, _lim.x_min, _x_scale, _ncol);
}

int Bucketing::ToIdxY(double y) const {
    return ::ToIdxX(y, _lim.y_min, _y_scale, _nrow);
}
